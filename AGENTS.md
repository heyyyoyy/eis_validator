...
├── Cargo.lock
├── .gitignore
├── certs/               ← gitignored; generated by generate_certs
│   ├── cert.pem
│   └── key.pem
├── schemas
...
- `src/bin/generate_certs.rs`: one-shot binary that generates `certs/cert.pem` and `certs/key.pem` using `rcgen`
- `src/main.rs`: bootstraps config, middleware, and HTTPS server startup/shutdown via `axum-server` + rustls
...
- `TLS_CERT` (default: `certs/cert.pem`) — path to the PEM certificate file
- `TLS_KEY` (default: `certs/key.pem`) — path to the PEM private key file

Example:

```bash
HOST=127.0.0.1 PORT=8080 LOG_LEVEL=debug cargo run
TLS_CERT=/etc/ssl/cert.pem TLS_KEY=/etc/ssl/key.pem cargo run
```
...
The server requires TLS certificates. Generate a self-signed certificate for local use:

```bash
cargo run --bin generate_certs
# Writes certs/cert.pem and certs/key.pem (gitignored)

# Optional: specify custom SANs
cargo run --bin generate_certs -- localhost 127.0.0.1 myhost.local
```
...
cargo build
cargo run
curl -k https://127.0.0.1:3000/health
curl -k -F "file=@your_file.xml" https://127.0.0.1:3000/validate
```
